...
export default class Experience {
  constructor(canvas) {
    ...
    const startZ = 20;
    const startX = 0;
    const xz = Math.sqrt(startX * startX + startZ * startZ);
    const angle = Math.atan2(xz, this.sphereRadius);
    const height = this.sphereRadius * Math.cos(angle);
    this.position = new THREE.Vector3(
      startX,
      -this.sphereRadius + height + Config.player.headHeight,
      startZ
    );
  ...
  updateMovement() {
    if (this.controls.isLocked) {
      ...
      // Apply only yaw rotation for movement direction
      ...
      if (moveDirection.length() > 0) {
        ...
        if (angle <= Config.world.visibleSectionAngle) {
          ...
          // Apply full rotation (yaw and pitch) to camera
          const qx = new THREE.Quaternion();
          const qy = new THREE.Quaternion();
          qx.setFromAxisAngle(new THREE.Vector3(0, 1, 0), this.yaw);
          qy.setFromAxisAngle(new THREE.Vector3(1, 0, 0), this.pitch);
